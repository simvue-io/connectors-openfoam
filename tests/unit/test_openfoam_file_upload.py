...
def test_openfoam_file_upload(folder_setup):
    ...
    with tempfile.TemporaryDirectory(prefix="openfoam_test") as temp_dir:
        ...
        client.get_artifacts_as_files(run_id, "code", temp_dir)
        ...
        for temp_subdir_path in temp_dir_path.rglob("./"):
            example_path = pathlib.Path(__file__).parent.joinpath(
                "example_data", "openfoam_case"
            )
...
def test_openfoam_file_upload_zipped(folder_setup):
    ...
    with tempfile.TemporaryDirectory(prefix="openfoam_test") as temp_dir:
        temp_dir_path = pathlib.Path(temp_dir)
        example_dir_path = pathlib.Path(__file__).parent.joinpath(
            "example_data", "openfoam_case"
        )
        client.get_artifacts_as_files(run_id, "input", temp_dir)
        client.get_artifacts_as_files(run_id, "output", temp_dir)
        ...
        for file in ["inputs", "results"]:
            file_path = pathlib.Path(temp_dir).joinpath(f"{file}.zip")
            assert file_path.is_file()
            # Extract files from zip file to directory to check contents
            with zipfile.ZipFile(file_path) as zip_file:
                zip_file.extractall(pathlib.Path(temp_dir).joinpath(file))

        dirs_to_compare = {
            example_dir_path.joinpath("system"): temp_dir_path.joinpath(
                "inputs", "system"
            ),
            example_dir_path.joinpath("constant"): temp_dir_path.joinpath(
                "inputs", "constant"
            ),
            example_dir_path.joinpath("0"): temp_dir_path.joinpath("inputs", "0"),
            example_dir_path.joinpath("0"): temp_dir_path.joinpath("results", "0"),
            example_dir_path.joinpath("0.003"): temp_dir_path.joinpath(
                "results", "0.003"
            ),
        }
