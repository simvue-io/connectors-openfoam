...
def mock_openfoam_process(self, *_, **__):
    ...
    def write_to_log():
        ...
        with (
            pathlib.Path(self.openfoam_case_dir)
            .joinpath("log.openfoam")
            .open(mode="w") as temp_logfile
        ):
            with (
                pathlib.Path(__file__)
                .parent.joinpath("example_data", "openfoam_log_initial.txt")
                .open("r") as log_file_initial
            ):
            ...
            with (
                pathlib.Path(__file__)
                .parent.joinpath("example_data", "openfoam_log_lines.txt")
                .open("r") as log_file_next
            ):
...
def test_openfoam_log_parser(folder_setup):
    ...
    with tempfile.TemporaryDirectory(prefix="openfoam_test") as temp_dir:
        pathlib.Path(temp_dir).joinpath("Allrun").touch()
        with OpenfoamRun() as run:
            run.config(disable_resources_metrics=True)
            run.init(name=name, folder=folder_setup)
            run_id = run.id
            run.launch(
                openfoam_case_dir=temp_dir,
            )
    ...
    assert (
        run_data.metadata["openfoam"]["allowsystemoperations"]
    ...
    sample_metric = client.get_metric_values(
        metric_names=["residuals.initial.p"],
        xaxis="time",
