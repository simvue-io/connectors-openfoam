...
To run this example with Docker:
    ...
    - Run the example script: python examples/movingcone_example.py
...
To run this example on your own system with Openfoam installed:
    ...
    - Run the example script: poetry run python movingcone_example.py
...
def openfoam_example(run_folder, offline=False):
    ...
    # Delete local copies of results generated by previous runs using the Allclean file
    subprocess.run(["/bin/sh", str(pathlib.Path(__file__).parent.joinpath("movingCone", "Allclean"))])
