...
To run this example with Docker:
    ...
    - Run the example script: python movingcone_example.py
...
To run this example on your own system with Openfoam installed:
    ...
    - Run the example script: poetry run python examples/movingcone_example.py
...
with OpenfoamRun() as run:
    ...
    run.init(
        name="openfoam_simulation_movingcone-%s" % str(uuid.uuid4()),
        folder="/examples/openfoam",
        tags=["openfoam", "moving_cone"],
    )
